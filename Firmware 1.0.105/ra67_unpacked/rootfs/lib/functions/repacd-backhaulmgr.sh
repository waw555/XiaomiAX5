...
__repacd_backhaulmgrmon_warn() {
    local stderr=''
    if [ "$BACKHAULMGRMON_DEBUG_OUTOUT" -gt 0 ]; then
        stderr='-s'
    fi
    logger $stderr -t repacd.backhaulmgrmon -p user.warn "$1"
}
...
repacd_backhaulmgrmon_init() {
    ...
    config_get eth_backhaul_mono 'BackhaulMgr' 'MiwifiEthBackBaulMonopoly' '1'
}

repacd_backhaulmgrmon_check() {
  if [ "$eth_backhaul_mono" -gt 0 -a "$miwifi_ethbh_mono_en" -gt 0 ]; then
        if [ ! "$is_wlan_ifaces_brought_down" -eq 1 ]; then
            __repacd_backhaulmgrmon_info " miwifi eth bh mono, shutdown wlan bhs $sta_iface_5g & $sta_iface_24g "
            [ -n $sta_iface_5g ] && __repacd_wifimon_bring_iface_down $sta_iface_5g
            [ -n $sta_iface_24g ] && __repacd_wifimon_bring_iface_down $sta_iface_24g
        fi
                bssid_assoc_count=0
                ...
                wlan_rate_consistent_num=0
        return
  elif [ "$select_only_one_backhaul_interface" -eq 0 -a "$miwifi_ethbh_mono_en" -eq 0 ]; then
        if [ "$is_wlan_ifaces_brought_down" -eq 1 ]; then
            __repacd_backhaulmgrmon_info " miwifi bring up wlan bhs"
    ...
    if [ "$select_only_one_backhaul_interface" -gt 0 ]; then
        if __repacd_plcmon_is_plc_enabled; then
            ...
            if __repacd_plcmon_is_plc_connected plc_iface_rate; then
                ...
                __repacd_backhaulmgrmon_debug "Rate sample: PLC to CAP = $plc_iface_rate Mbps"
                ...
                if [ "$curr_root_distance" -gt 1 -a \
                    ...
                    if [ "$bssid_assoc_count" -ge "$min_bssid_assoc" ]; then
                        ...
                        if [ "$wlan_rate_consistent_num" -eq "$backhaulmgr_rate_samples" ]; then
                            __repacd_backhaulmgrmon_warn "Daisy, Bringing down PLC interface..."
                            ...
                            __repacd_backhaulmgrmon_info "Daisy, Bringing down WLAN interface..."
                ...
                if [ "$is_wlan_ifaces_brought_down" -eq 1 ]; then
                    ...
                    if [ "$prev_wlan_rate" -gt "$plc_iface_rate" ]; then
                        ...
                        if [ "$prev_wlan_rate_compare_counter" -eq "$backhaulmgr_rate_samples" ]; then
                            __repacd_backhaulmgrmon_warn " Daisy, PLC Link rate ($plc_iface_rate Mbps) \
is bad than previous rate of WLAN Link ($prev_wlan_rate Mbps)"
                            ...
                            if [ "$bssid_assoc_count" -gt 0 ]; then
                                ...
                                __repacd_backhaulmgrmon_warn " Daisy, Bringing up WLAN interface..."
                    ...
                    if [ "$gw_not_reachable_consistent_num" -eq "$PingTimeoutsToCAP" ]; then
                        ...
                        if [ "$bssid_assoc_count" -gt 0 ]; then
                            ...
                            # Only bringup WLAN interfaces if WLAN interfaces are not up
                            __repacd_backhaulmgrmon_debug "Bringing up WLAN interface... 2"
                            __repacd_wifimon_bring_iface_up $sta_iface_5g
                            __repacd_wifimon_bring_iface_up $sta_iface_24g
                        fi
                        is_wlan_ifaces_brought_down=0
                    ...
                    if [ "$curr_root_distance" -gt 1 -a \
                        ...
                        __repacd_backhaulmgrmon_info "daisy-chain, GW ($gw_ip) not reachable, leave wifi & Bringing down PLC interface..."
                        __repacd_plcmon_bring_plc_iface_down
                        is_plc_iface_brought_down=1
                        prev_plc_rate=0
                        __repacd_wifimon_get_timestamp force_down_plc_timestamp
                ...
                if [ "$is_wlan_ifaces_brought_down" -eq 1 -a \
                    ...
                    __repacd_backhaulmgrmon_debug "Bringing up WLAN interface... 1"
                    __repacd_wifimon_bring_iface_up $sta_iface_5g
                    __repacd_wifimon_bring_iface_up $sta_iface_24g
                    is_wlan_ifaces_brought_down=0
                ...
                # Check whether PLC is brought down by repacd
                    ...
                    if [ "$curr_root_distance" -eq "$RE_ROOT_AP_DISTANCE_INVALID" ] || \
                       [ "$curr_root_distance" -eq 1 ]; then
                        ...
                        __repacd_backhaulmgrmon_warn "Bringing up PLC:eval interval $config_eval_time_plc sec expired"
                        __repacd_plcmon_bring_plc_iface_up
                        is_plc_iface_brought_down=0
                        __repacd_wifimon_get_timestamp force_down_plc_timestamp
                    fi
                    ...
                    if [ "$curr_root_distance" -gt 1 ]; then
                        ...
                        if [ "$rate_to_CAP" -eq 0 -a "$prev_plc_rate" -lt "$plc_link_threshold_to_2G" ]; then
                            ...
                            if [ "$prev_plc_rate_compare_counter" -eq "$backhaulmgr_rate_samples" ]; then
                                __repacd_backhaulmgrmon_warn "Daisy WLAN Link rate ($rate_to_CAP Mbps) \
is bad than previous rate of PLC Link ($prev_plc_rate Mbps)"
                                __repacd_backhaulmgrmon_warn "Bringing up PLC interface..."
                                __repacd_plcmon_bring_plc_iface_up
                                is_plc_iface_brought_down=0
                                prev_plc_rate_compare_counter=0
                            fi
