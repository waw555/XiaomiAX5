...
if [ -z "$old_version" ] && [ "$netmod" = "whc_cap" -o "$netmod" = "whc_re" ]; then
    ...
    backhaul_sta_5g=$(uci -q get misc.backhauls.backhaul_5g_sta_iface)
    iface_sta_5g=$(uci show wireless | grep -w "ifname=\'$backhaul_sta_5g\'" | awk -F"." '{print $2}')
    bssid_set=$(uci -q get wireless.$iface_sta_5g.bssid)
    if [ -n "$bssid_set" ]; then
        uci delete wireless.$iface_sta_5g.bssid
    fi
    uci commit wireless
