...
LAST = "$(eval $EPOCH_TIME)"
uci -q set ddns.$SECTION_ID.lastupdate="$LAST"
uci -q commit ddns
write_log 7 "last update: ==== $LAST"
fi
...
write_log 3 "Verbose Mode: $VERBOSE - NO UPDATE send"
elif [ "$LOCAL_IP" != "$REGISTERED_IP" ]; then
write_log 3 "Update needed - L: '$LOCAL_IP' <> R: '$REGISTERED_IP'"
else
write_log 3 "Forced Update - L: '$LOCAL_IP' == R: '$REGISTERED_IP'"
fi
ERR_LAST=0
[ $VERBOSE -lt 3 ] && {
...
&& write_log 3 "Update successful - IP '$LOCAL_IP' send" \
|| write_log 3 "Forced update successful - IP: '$LOCAL_IP' send"
...
uci -q set ddns.$SECTION_ID.laststatus="loading"
uci -q commit ddns
REGISTERED_IP=""
...
write_log 3 "Updating IP at DDNS provider failed - starting retry $ERR_UPDATE/$retry_count"
continue
else
write_log 3 "Updating IP at DDNS provider failed"
write_log 7 "Verbose Mode: $VERBOSE - NO retry"; exit 1
fi
else
write_log 3 "IP check ok local ip == registered/public IP."
uci -q set ddns.$SECTION_ID.laststatus="ok"
uci -q commit ddns
ERR_UPDATE=0
fi
