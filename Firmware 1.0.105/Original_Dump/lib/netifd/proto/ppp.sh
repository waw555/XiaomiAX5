...
ppp_generic_init_config() {
    ...
    # parameters: persist,maxfail controlled by option-files seperately
...
ppp_generic_setup() {
	...
	json_get_vars ipv6 ip6table demand keepalive keepalive_adaptive username password pppd_options pppname unnumbered holdoff peerdns
	...
	# set LCP echo default: interval=20s, fail=4
	[ -n "$keepalive" ] || keepalive="4 20"
...
proto_pppoe_setup() {
	...
	local specdial
	local special='/etc/ppp/pppoe_special'
	[ -f "$special" ] && specdial=$(cat "$special")
...
proto_pptp_setup() {
    ...
    json_get_var mru mru
    mru="${mru:-1410}"	
    #adjust MTU/MRU by wan's MTU
    local wan_mtu=$(get_wan_mtu)
    [ -n "$wan_mtu" ] && {
        wan_mtu=`expr $wan_mtu - 60`    #IP+TCP+GRE+PPP
        [ "$wan_mtu" -lt "$mru" ] && mru=$wan_mtu
    }
