...
update_daemon_coldboot_qdss_support_variables() {
	local board_name=""
	local board_prefix=""
	local version=0
	local fw_ini_file=/lib/firmware/firmware_rdp_feature.ini
	...
	[ -e /sys/firmware/devicetree/base/MP_256 ] && {
		echo "****** No Coldboot/QDSS Support for low mem ******" > /dev/console
		return
	}
	...
	ap-hk*)
		[ -f /sys/firmware/devicetree/base/soc_version_major ] && {
			version="$(hexdump -n 1 -e '"%1d"' /sys/firmware/devicetree/base/soc_version_major)"
		}

		local default_string=$board_prefix"_v"$version"_default"
		;;
	*)
		local default_string=$board_prefix"_default"
		;;
	esac

	#Grep the board_name in the fw_ini_file, if its not found, use the
	#default string
	boardname_grep_result=`grep -ci $board_name $fw_ini_file`
	default_grep_result=`grep -ci $default_string $fw_ini_file`

	if [ $boardname_grep_result != 0 ]
	then
		daemon=`grep -i $board_name $fw_ini_file | grep "enable_daemon_support" | awk -F'=' '{print $2}' | cut -c 1`
		cold_boot_support=`grep -i $board_name $fw_ini_file | grep "enable_cold_boot_support" | awk -F'=' '{print $2}' | cut -c 1`
		qdss_tracing=`grep -i $board_name $fw_ini_file | grep "enable_qdss_tracing" | awk -F'=' '{print $2}' | cut -c 1`
	elif [ $default_grep_result != 0 ]
	then
		daemon=`grep -i $default_string $fw_ini_file | grep "enable_daemon_support" | awk -F'=' '{print $2}' | cut -c 1`
		cold_boot_support=`grep -i $default_string $fw_ini_file | grep "enable_cold_boot_support" | awk -F'=' '{print $2}' | cut -c 1`
		qdss_tracing=`grep -i $default_string $fw_ini_file | grep "enable_qdss_tracing" | awk -F'=' '{print $2}' | cut -c 1`
	else
...
enable_qdss_tracing() {
	...
	ap-cp*)
		[ ! -f /tmp/qdss_trace_commands ] && {
			echo "q6mem" > /sys/bus/coresight/devices/coresight-tmc-etr/out_mode
			echo 1 > /sys/bus/coresight/devices/coresight-tmc-etr/curr_sink
			echo "0x06041FB0 0xc5acce55" > /sys/bus/coresight/devices/coresight-hwevent/setreg
			echo "0x06130FB0 0xc5acce55" > /sys/bus/coresight/devices/coresight-hwevent/setreg
			echo "0x06041000 0x00000320" > /sys/bus/coresight/devices/coresight-hwevent/setreg
			echo "0x06130000 0x00000340" > /sys/bus/coresight/devices/coresight-hwevent/setreg
			echo 1 > /sys/bus/coresight/devices/coresight-stm/enable
			echo "*****Registers configuration for qdss_tracing completed*******" > /dev/console
		}
...
do_cold_boot_calibration() {
	...
	#daemon and cold_boot_support are assigned a value globally
	...
	[ $daemon = 1 ] && {
		echo "*****starting cnssdaemon*****" > /dev/console
		/usr/bin/cnssdaemon &
	}
	...
	pid=`pgrep cnssdaemon` && echo -1000 > /proc/$pid/oom_score_adj
	echo "*****cnssdaemon pid=$pid*********" > /dev/console
...
{
	update_internal_ini $1 dp_rxdma_monitor_buf_ring 16
	update_internal_ini $1 dp_rxdma_monitor_dst_ring 16
	update_internal_ini $1 dp_rxdma_monitor_desc_ring 16
	...
	if [ "$mp_256" == "MP_256" ]; then
		...
		[ -d /sys/module/qca_ol ] || { \
			...
			if [ $hk_ol_num -eq 3 ]; then
				...
				sysctl -w dev.nss.n2hcfg.extra_pbuf_core0=3200000 >/dev/null 2>/dev/null
				sysctl -w dev.nss.n2hcfg.n2h_high_water_core0=31648 >/dev/null 2>/dev/null
				sysctl -w dev.nss.n2hcfg.n2h_wifi_pool_buf=0 >/dev/null 2>/dev/null
			else
...
enable_qcawificfg80211() {
	...
	for vif in $vifs; do
		...
		config_get_bool vap_contryie "$vif" vap_contryie 1
...
#TODO
		...
		# miwifi disable wnm as default
		config_get wnm "$vif" wnm 0
		...
		config_get maxsta "$vif" maxsta 64
...
#TODO
		...
		# for miwifi
		config_get_bool ap_isolate "$vif" ap_isolate 0
        ...
        if [ "$bdmode" = "24G" ]; then
            max_power=28
            # miwifi enable dynamic edcca
            wifitool "$ifname" setUnitTestCmd 67 3 16 1 1
            # miwifi enable rtscts rate below MCS7
            wifitool "$ifname" setUnitTestCmd 0x48 2 192 7
            "$device_if" "$ifname" enablertscts 0x11
            # miwifi ANI level rise up when there is more than 15% PHY ERROR
            wifitool "$ifname" setUnitTestCmd 67 5 12 1 -5 15 15
        else
            max_power=27
        fi
...
detect_qcawificfg80211() {
	...
	is_e_build=0
	country_code="CN"
	bd_country_code=`bdata get CountryCode`
	nv_country_code=`nvram get CountryCode`
	if [ "$bd_country_code" == "US" ]; then
		country_code="US"
	else 
	...
	for dev in $(ls -d wifi* 2>&-); do
        ...
        devidx24g=1
		ssid=`nvram get wl${devidx}_ssid`
		[ -z "${ssid}" ] && ssid='test'
		if [ $devidx = 0 ]; then
			htmode=HT80
		else 
...
EOF
	...
	if [ "$is_ftm" = "1" ] && [ "$wl0_radio" != "1" ] && [ "$wl1_radio" != "1" ]; then
		echo "Enter FTM Mode" > /dev/kmsg
		rmmod wifi_3_0 > /dev/null
		rmmod qca_ol > /dev/null
		insmod qca_ol testmode=1 > /dev/null
		insmod wifi_3_0 > /dev/null
	fi
}
