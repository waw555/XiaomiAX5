...
hostapd_set_bss_options() {
	...
	local hardware="`nvram get model`"
	...
	config_get wpa_pair_rekey   "$vif" wpa_pair_rekey 0  # 300
	config_get wpa_master_rekey "$vif" wpa_master_rekey 0 # 640
			...
			dpp|sae*|psk2|wpa2)
				...
				then
					...
					then
						...
						#miwifi support CCMP only on psk2, otherwise mi6 cannot connect
						crypto="CCMP"
					else
						crypto="CCMP"
					fi
	...
	# miwifi default push_button, so modify 1
	config_get_bool wps_pbc "$vif" wps_pbc 1
	...
	then
		...
		then
			...
			# miwifi As xiaomi don't need to support sae password list,
			# and set "option" in lua script, parse "option" here instead of "list"
			#add_sae_passwords() {
	...
	if [ $MapBSSType -eq 96 ]; then
		append "$var" "multi_ap=3" "$N"
	elif [ $MapBSSType  -eq 32 ]; then
		append "$var" "multi_ap=2" "$N"
	elif [ $MapBSSType -eq 64 ]; then
...
hostapd_setup_vif() {
	...
	if [ $channel -eq 36 ]; then
		if [ $edge_channel_deprioritize -eq 1 ]; then
			case "$board_name" in
				ap-hk*|ap-ac*|ap-oa*)
				[ 36 = "$channel" ] && [ HT20 != "$htmode" ] && channel=40 && echo " Primary channel is changed to 40"
