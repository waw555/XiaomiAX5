...
WHC_LOG="/tmp/whc.log"   ### for D01 xqwhc
GZ_LOGS=""
...
echo "==========arp" >> $LOG_TMP_FILE_PATH
cat /proc/net/arp >> $LOG_TMP_FILE_PATH

[ -f /usr/sbin/showmacs ] && {
    echo "==========showmacs" >> $LOG_TMP_FILE_PATH
    showmacs >> $LOG_TMP_FILE_PATH
}
...
estimator s, steeralg s, steerexec s, ps s, ps p, ps f" "$WHC_LOG"
...
elif [ "$hardware" = "RM1800" ] || [ "$hardware" = "R3600" ] || [ "$hardware" = "RA67" ]; then
    netmode="`uci -q get xiaoqiang.common.NETMODE`"
    log_exec "iwconfig >> $LOG_TMP_FILE_PATH"
    for i in `seq 0 1`; do
        log_exec "athstats -i wifi$i >> $LOG_TMP_FILE_PATH"
    done
    list="0 1"
    ...
    for i in $list; do
        ...
        log_exec "wlanconfig wl$i list >> $LOG_TMP_FILE_PATH"
        log_exec "80211stats -a -i wl$i >> $LOG_TMP_FILE_PATH"
        log_exec "iwlist wl$i channel >> $LOG_TMP_FILE_PATH"
        log_exec "iwpriv wl$i get_chutil >> $LOG_TMP_FILE_PATH"
        log_exec "iwpriv wl$i get_channf >> $LOG_TMP_FILE_PATH"
        log_exec "iwpriv wl$i txrx_stats 1"
        log_exec "iwpriv wl$i txrx_stats 3"
...
log_exec "[ -f /tmp/vpn.stat.msg ] && cat /tmp/vpn.stat.msg"
...
echo "    trafficd ma_dump_status:" >> $TRAFFICD_LOG
ubus call trafficd ma_dump_status >> $TRAFFICD_LOG
...
move_files="$LOG_TMP_FILE_PATH $IPTABLES_SAVE $TRAFFICD_LOG $PLUGIN_LOG $NETWORK_STRIP $WIRELESS_STRIP $MICLOUD_LOG $NVRAM_FILE_PATH $BDATA_FILE_PATH"
...
[ -d "$PPP_LOG" ] || PPP_LOG=
dup_files="$DHCP_LEASE $DNSMASQ_CONF $MACFILTER_FILE_PATH $CRONTAB $QOS_CONF $WIFISHARE_CONF $PPP_LOG "
...
[ "$hardware" = "D01" -o "$hardware" = "RM1800" -o "$hardware" = "R3600" -o "$hardware" = "RA67" ] && {
...
tar -zcvf $LOG_ZIP_FILE_PATH $move_files $dup_files $redundancy_files $CONFIG_TMP_DIR
