...
__gwmon_prevent_island_loop() {
    ...
    while [ "$retries" -gt 0 ]; do
        ...
        if ping -W 2 "$gw_ip" -c1 > /dev/null; then
            ...
            retries=$((retries -1))
            __gwmon_warn "Ping to GW IP[$gw_ip] failed ($retries retries left)"
        fi
    done
...
__gwmon_check_gw_iface_link() {
    ...
    if [ "$interf" -gt 0 ]; then
        ...
        if [ ! "$link_status" = "down" ]; then
            ...
            [ $? -eq 0 ] && gw_unreachable=0 || {
                gw_unreachable=$((gw_unreachable +1))
                if [ $gw_unreachable -ge $MAX_UNREACHABLE ]; then
                    __gwmon_warn "Checking $gw_iface is up and running, but ping failed for $gw_unreachable times, GW unreachable"
                    gw_unreachable=0
                    return 1
                else
                    __gwmon_warn "Checking $gw_iface is up and running, but ping failed for $gw_unreachable times"
                fi
            }
            return 0
        fi
...
__update_gw_reachable_status() {
    local gw_ip=$1
    if ping -W 2 "$gw_ip" -c1 > /dev/null; then
        if [ "$IS_GW_REACHABLE" -eq 0 ]; then
            __gwmon_info "Recheck GW ($gw_ip) reachable"
        fi
        IS_GW_REACHABLE=1
        GW_NOT_REACHABLE_TIMESTAMP=0
    else
        if [ "$IS_GW_REACHABLE" -eq 1 ]; then
            __gwmon_info "Recheck GW ($gw_ip) NOT reachable"
        fi
        IS_GW_REACHABLE=0
...
__gwmon_check_gateway() {
    ...
    bridge_eth_ifaces="$ether_ifaces"
    __gwmon_debug " @@ gw_check_info: $gw_br_port;$__gw_iface; $ether_ifaces; $gw_ip;$gw_mac"
    ...
    for ether_iface in $ether_ifaces; do
        if [ "$ether_iface" = "$__gw_iface" ]; then
            ...
            if [ "$IS_GW_REACHABLE" -eq 0 ]; then
                __gwmon_info "recheck and update gw reachable status $gw_ip"
                __update_gw_reachable_status $gw_ip
            fi
            return 0
        fi
    done
...
__gwmon_miwifi_eth_bh_exist()
{
        if [ "$router_detected" -gt 0 ]; then
            ...
            if [ "$mixedbh" != "1" ]; then
                ...
                __gwmon_info " miwifi gwmon_check, find eth bh on $gw_iface, shutdown wlan & plc bhs @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@ "
                return $GWMON_MODE_NON_CAP_ETH
            fi
        fi
